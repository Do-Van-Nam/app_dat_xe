...
class _MapBackgroundState extends State<MapBackground> {
  ...
  void didUpdateWidget(MapBackground oldWidget) {
    ...
    if (widget.driverPoint != oldWidget.driverPoint ||
        widget.isShowDriverPoint != oldWidget.isShowDriverPoint) {
      _updateDriverMarker();
    }
  }
    ...
    try {
      ...
      try {
        final ByteData driverBytes =
            await rootBundle.load('assets/images/img_driver.png');
        await _mapController?.addImage(
            'driverLocation', driverBytes.buffer.asUint8List());
      } catch (_) {
        await _mapController?.addImage(
            'driverLocation', originBytes.buffer.asUint8List());
      }
    } catch (_) {
  ...
  Future<void> _updateDriverMarker() async {
    if (_mapController == null || !_imagesLoaded) return;
    final isShow = widget.isShowDriverPoint ?? false;
    final dPoint = widget.driverPoint;

    if (!isShow || dPoint == null) {
    ...
    try {
      if (_driverMarker != null) {
        await _mapController!.updateSymbol(
          _driverMarker!,
          SymbolOptions(geometry: dPoint),
        );
      } else {
        _driverMarker = await _mapController!.addSymbol(SymbolOptions(
          geometry: dPoint,
          iconImage: 'driverLocation',
          iconSize: 1.3,
          zIndex: 998, // Dưới origin và destination một chút
        ));
      }
    } catch (_) {}
  }
